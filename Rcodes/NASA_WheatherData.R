...
for (i in 1:nrow(info_loc)) {
  ...
  tmp <- as.data.frame(get_power(community = 'AG',
                                 ...
                                 dates = c(info_loc$sowing[i]-60, info_loc$harvesting[i]+60),
  ...
  tmp3 <- data.frame(tmp3,tmp=NA); colnames(tmp3)[ncol(tmp3)] <- toupper(attributes)
  for(dd in 1:nrow(tmp3)){
    tmp <- tmp4[tmp4$YYYYMMDD %in% as.character(tmp3$YYYYMMDD[dd]),]
    tmp3[dd,toupper(attributes)] <- sum(tmp[,toupper(attributes)])*0.0036    #(1WH = 0.0036 MJ)
  }
